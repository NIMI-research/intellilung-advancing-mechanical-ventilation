...
def eval_checkpoint_ood(dataset_config, pre_processing_configs, ae_dataset_config, device, checkpoint, ae_checkpoint,
    ...
    evaluator = AgentEvaluator(experiment_path=ae_configs.experiment_path)
    evaluator.eval(checkpoint_id=policy_id, agent=agent, panel_name='Autoencoder_Eval', eval_buffer=buffer)

    individual_action_losses = eval_actions(model=agent.model, eval_buffer=buffer,
                                            cont_action_list=ae_cont_actions_list, batch_size=agent.batch_size)

    os.makedirs(extra_info_save_path, exist_ok=True)
    save_json(individual_action_losses, path=join(extra_info_save_path, 'individual_action_losses.json'))
    print(policy_id, individual_action_losses)
    losses_df = eval_get_all_losses(agent=agent, eval_buffer=buffer)
    losses_df.to_csv(join(extra_info_save_path, 'ae_losses.csv'), index=False)

    # def update_path(path, new_title):
    #     new_path = path.split('/')
    #     new_path[-2] = new_title
    #     return '/'.join(new_path)
    #
    # buffer_mode = convert_buffer_actions_to_discrete_and_back(
    #     buffer=buffer,
    #     cont_actions_list=ae_cont_actions_list,
    #     dataset_config=dataset_config,
    #     ae_dataset_config=ae_dataset_config,
    #     device=device,
    #     disc_to_cont_fn=disc_to_cont_using_mode
    # )
    #
    # evaluator = AgentEvaluator(
    #     experiment_path=update_path(path=ae_configs.experiment_path, new_title='Hybrid-Autencoder-Mode'))
    # evaluator.eval(checkpoint_id=policy_id, agent=agent, panel_name='Autoencoder_Eval_Mode',
    #                eval_buffer=buffer_mode)
    # losses_df = eval_get_all_losses(agent=agent, eval_buffer=buffer_mode)
    # losses_df.to_csv(join(extra_info_save_path, 'ae_losses_mode.csv'), index=False)
    #
    # buffer_uniform = convert_buffer_actions_to_discrete_and_back(
    #     buffer=buffer,
    #     cont_actions_list=ae_cont_actions_list,
    #     dataset_config=dataset_config,
    #     ae_dataset_config=ae_dataset_config,
    #     device=device,
    #     disc_to_cont_fn=disc_to_cont_using_uniform
    # )
    # evaluator = AgentEvaluator(
    #     experiment_path=update_path(path=ae_configs.experiment_path, new_title='Hybrid-Autencoder-Uniform'))
    # evaluator.eval(checkpoint_id=policy_id, agent=agent, panel_name='Autoencoder_Eval_Uniform',
    #                eval_buffer=buffer_uniform)
    # losses_df = eval_get_all_losses(agent=agent, eval_buffer=buffer_uniform)
    # losses_df.to_csv(join(extra_info_save_path, 'ae_losses_uniform.csv'), index=False)
    #
    # buffer_gauss = convert_buffer_actions_to_discrete_and_back(
    #     buffer=buffer,
    #     cont_actions_list=ae_cont_actions_list,
    #     dataset_config=dataset_config,
    #     ae_dataset_config=ae_dataset_config,
    #     device=device,
    #     disc_to_cont_fn=disc_to_cont_using_gauss
    # )
    #
    # evaluator = AgentEvaluator(
    #     experiment_path=update_path(path=ae_configs.experiment_path, new_title='Hybrid-Autencoder-Gauss'))
    # evaluator.eval(checkpoint_id=policy_id, agent=agent, panel_name='Autoencoder_Eval_Gauss',
    #                eval_buffer=buffer_gauss)
    # losses_df = eval_get_all_losses(agent=agent, eval_buffer=buffer_gauss)
    # losses_df.to_csv(join(extra_info_save_path, 'ae_losses_gauss.csv'), index=False)
    #
    # buffer_mean = convert_buffer_actions_to_discrete_and_back(
    #     buffer=buffer,
    #     cont_actions_list=ae_cont_actions_list,
    #     dataset_config=dataset_config,
    #     ae_dataset_config=ae_dataset_config,
    #     device=device,
    #     disc_to_cont_fn=disc_to_cont_using_mid
    # )
    #
    # evaluator = AgentEvaluator(
    #     experiment_path=update_path(path=ae_configs.experiment_path, new_title='Hybrid-Autencoder-Mean'))
    # evaluator.eval(checkpoint_id=policy_id, agent=agent, panel_name='Autoencoder_Eval_Mean',
    #                eval_buffer=buffer_mean)
    # losses_df = eval_get_all_losses(agent=agent, eval_buffer=buffer_mean)
    # losses_df.to_csv(join(extra_info_save_path, 'ae_losses_mean.csv'), index=False)
