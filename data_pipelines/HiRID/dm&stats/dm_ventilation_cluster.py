...
from dm_ventilation import process_patient
...
def process_batch(startid, stopid):
    ...
    for pid in range(startid,stopid+1):
        ...
        ds_merged = pl.scan_parquet(os.path.join(SOURCE_PATH, '*.parquet')).filter((pl.col('PatientID')==pid)).collect().to_pandas()
...
if __name__=="__main__": 
    ...
    # CONSTANTS 
    OUTPUT_DIR = '/cluster/work/grlab/clinical/hirid2/research/faltysm/Intellilung/dm_ventilation'
    LOG_DIR="/cluster/work/grlab/clinical/hirid2/research/faltysm/ICU_pipe/logs"
    SOURCE_PATH="/cluster/work/grlab/clinical/hirid2/research/faltysm/Intellilung/dm_merged/latest" 
    ...
    if args.run_mode=="CLUSTER":
        sys.stdout=open(os.path.join(LOG_DIR,"{}_Ventilation_{}_{}.stdout".format(datetime.today().strftime('%Y-%m-%d'), args.pid_start,args.pid_stop)),'w')
        sys.stderr=open(os.path.join(LOG_DIR,"{}_Ventilation_{}_{}.stderr".format(datetime.today().strftime('%Y-%m-%d'), args.pid_start,args.pid_stop)),'w')
