...
from dm_vasoactive import process_patient
...
if __name__=="__main__": 
    ...
    # CONSTANTS 
    OUTPUT_DIR = '/cluster/work/grlab/clinical/hirid2/research/faltysm/Intellilung/dm_vasoactive'
    ...
    if args.run_mode=="CLUSTER":
        sys.stdout=open(os.path.join(LOG_DIR,"{}_Vasoactive_{}_{}.stdout".format(datetime.today().strftime('%Y-%m-%d'), args.pid_start,args.pid_stop)),'w')
        sys.stderr=open(os.path.join(LOG_DIR,"{}_Vasoactive_{}_{}.stderr".format(datetime.today().strftime('%Y-%m-%d'), args.pid_start,args.pid_stop)),'w')
