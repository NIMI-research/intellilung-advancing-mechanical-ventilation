...
(
  select
      ...
      -- If the time since the last oxygen therapy event is more than 6 hours,
	...
	-- That is, the next ventilation record corresponds to a new ventilation session.
      , CASE
		WHEN (charttime - charttime_lag) > 6*60 THEN 1
...
(
	SELECT icustay_id
		...
		, unitdischargestatus
		, CASE
  		-- Apply only to the last episode (ventnum > 1) where the patient is marked as "Expired"
      WHEN (unitdischargestatus = 'Expired' 
           AND ventnum = MAX(ventnum) OVER (PARTITION BY icustay_id))
      THEN MAX(pat.unitdischargeoffset)
			...
			WHEN (
				...
				AND MAX(charttime) < MAX(pat.unitdischargeoffset)
			)
			THEN MAX(pat.unitdischargeoffset)
	...
	GROUP BY icustay_id, ventnum, unitdischargestatus
),
    vent_table AS(
    select vd3.*
...
from vd3),
    ...
    sq1 AS(/*New code begins here*/
    SELECT
      *,
      pat.unitdischargestatus AS pat_unitdischargestatus, 
      PARSE_TIMESTAMP ('%Y-%m-%d %H:%M:%S', '2023-01-01 ' || unitadmittime24) AS intime /*Assign the unit admit day to 2023-01-01 to operate with timestamp format*/
    FROM
      vent_table
    INNER JOIN
      pat
    ON
      vent_table.icustay_id = pat.patientunitstayid
    WHERE
    ...
    SELECT
      ...
      intime,
      TIMESTAMP_ADD(intime, INTERVAL unitdischargeoffset MINUTE) AS outtime /*Calculates the ICU outtime timedate from intime and unitdischargeoffset*/,
      pat_unitdischargestatus AS unitdischargestatus,
      unitvisitnumber,
      ventnum,
      vent_start,
      starttime,
    ...
    FROM (
      SELECT
        *,
        TIMESTAMP_ADD(intime, INTERVAL vent_start MINUTE) AS starttime /*Add the minutes of ventilation start to intime*/,
        TIMESTAMP_ADD(intime, INTERVAL vent_end MINUTE) AS endtime /*Add the minutes of ventilation end to intime*/
      FROM
        sq1)
    ORDER BY
      stay_id,
